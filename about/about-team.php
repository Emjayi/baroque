...
$aboutTeamArray =  glob($teamPath.'*',GLOB_ONLYDIR);

// sort team folders by name 
$aboutTeamArray1 = array();
foreach ( $aboutTeamArray as $teamFolderArrayItem )
	array_push($aboutTeamArray1,$teamFolderArrayItem);
$aboutTeamArray = $aboutTeamArray1;

$aboutTeamLen   =  sizeof($aboutTeamArray);
...
// ------------- main loop 
for ($index=0 ; $index < $aboutTeamLen ; $index++ ) {
	
	if ( $sortBYFile ) $i = $sortArray[$index];
	else $i = $index;

	$member = $aboutTeamArray[$i];
		...
		<li class="team-item">
			<div class="team-text">
				...
				<p class="team-info">
					...
					<span>
					<?php
						echo 'since '; e($memberYear);
						if ($memberHasLink) {?> 
						<a
						   href="<?php e($memberSoc) ?>"
						   title="<?php e($memberName) ?>&#39;s instagram page"
						   target="_blank"
						><?php echo $instaIcon ?></a>
					<?php } ?> 
					</span>
				</p>
				<div class="team-top">
					<div class="team-number">
						<?php
							if ( $index == 0 ) echo $logoMark;
							else echo $logoMarkCopy;
							echo ($index + 1);
						?> 
					</div>
			...
			<figure class="team-img <?php if($index > $lazyIndex)echo'lazy';?>">
				<div class="team-img-paralex">
					<!-- normal image -->
					<img
						data-src="<?php echo $memberImage ?>" 
						src="<?php
							if ($index > $lazyIndex) echo $loading_img;
							else echo $memberImage; ?>"
						alt="<?php e($memberName) ?>"
						class="team-img-main"
					/>
					<!-- smile image -->
					<img
						data-src="<?php echo $memberImageHover ?>"
						src="<?php
							if ($index > $lazyIndex) echo $loading_img;
							else echo $memberImageHover; ?>"
						alt="<?php e($memberName) ?>, smiling"
		...
		<li class="team-item team-item-job" id="job-btn">
			...
			<div class="team-img team-img-job" aria-hidden="true">
				<div class="team-img-paralex team-img-job">
					...
					<!-- people picture -->
					<svg
						class="job-icon-svg" role="img"
						xmlns="http://www.w3.org/2000/svg" viewBox="-5 -5 458 522" >
						<path d="M224 256c70.7 0 128-57.3 128-128S294.7 0 224 0 96 57.3 96 128s57.3 128 128 128zm89.6 32h-16.7c-22.2 10.2-46.9 16-72.9 16s-50.6-5.8-72.9-16h-16.7C60.2 288 0 348.2 0 422.4V464c0 26.5 21.5 48 48 48h352c26.5 0 48-21.5 48-48v-41.6c0-74.2-60.2-134.4-134.4-134.4z" class="team-svg"></path>
					</svg>
				</div>
			</div>
		</li>