...
if ( !isset($_POST['section']) ) {
	if ( $ajax ) echo "$sender_php?upload=$upload_status";
	else header("Location: $sender_php?upload=$upload_status"); 
	die();
}
...
if ( $_FILES["upload_file"]["error"] == 1 ) {
	if ( $ajax ) echo "$sender_php?upload=errorUploading";
	else header("Location: $sender_php?upload=errorUploading"); 
	die();
}
/*echo ( ' - name - '.
...
if ( $fileUnzipped ) {
	...
	// $sort_File is denited in the begining;
	if ( file_exists($sort_File) ) {
		$sortFileData = file( $sort_File , FILE_IGNORE_NEW_LINES);
		$sortFileDataLen = sizeof($sortFileData);
		array_push( $sortFileData , $sortFileDataLen ) ;
		$newSortArray = implode(PHP_EOL,$sortFileData);
