...
<!-- options button -->
<button
	class="hide-until-load header-button options-button"
	id="options-btn" onclick="this.blur();">
	<span class="header-button-txt" >options</span>
	<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16">
		<path d="M16 7v2h-2.7V7H16zM12 6H9v1H0v2h9v1h3V6zM16 2v2H8.3V2H16zM7 1H4v1H0v2h4v1h3V1zM16 12v2H6.3v-2H16zM5 11H2v1H0v2h2v1h3v-4z"/>
	</svg>
</button>
	...
	<div class="grid-parent-con">
		...
		for ( $index = 0; $index < $projectsFolderCount; $index++ ) {
			...
			<li data-index="<?php echo $index; ?>"
				class="grid-item <?php e($arch_status)?> <?php e($arch_type)?> <?php if( $index > $lazyindex) echo'lazy'; ?>"
				data-area="<?php echo $arch_area; ?>"
				data-width="<?php calculateImgRatio($tmbImage[0]) ?>"
				>
				<a  href="<?php echo $project_link; ?>"
					class="grid-link smoooth">
					<div class="grid-img">
						<img src="<?php if( $index > $lazyindex) echo $loading_img; else echo $imgPath; ?>"
							 data-src="<?php echo $imgPath; ?>"
							 alt="<?php echo $grid_title; ?>"
							 <?php if( $index > $lazyindex) echo 'class="lazy-img"'; ?>
						/>
